...
int main()
{
    ...
    for (int t = 10; t <= 15; t++)
    {
        ...
        vector<int> a;
        for (int i=0; i<n; i++) a.push_back(NoZeroRandom(10000));      
        ...
        n = a.size();
        int x = NoZeroRandom(n-1), y = NoZeroRandom(n-1), z = NoZeroRandom(n-1),k = NoZeroRandom(n-1);
        int target = a[x] + a[y] + a[z] + a[k];//三数之和
        //输出target,n
