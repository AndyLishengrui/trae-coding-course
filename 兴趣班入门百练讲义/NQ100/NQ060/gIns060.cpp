...
int main() {
  ...
  for (int t = 3; t <= 10; t++) {
    ...
    n = NoZeroRandom(1000);
    vector<int> a(n);
    for (int i = 0; i < n; i++) a.push_back(NoZeroRandom(10000000000));
    sort(a.begin(), a.end());
    a.erase(unique(a.begin(), a.end()), a.end());
    n = a.size();
    int x,y,x1,y1;
    do {
      x = NoZeroRandom(n - 1), y = NoZeroRandom(n - 1);
      x1 = NoZeroRandom(n - 1), y1 = NoZeroRandom(n - 1);
    } while (x==x1 || x== y|| x==x1 || x == y1 || y == x1|| y==y1 || x1 == y1);


    int target = a[x] + a[y] + a[x1] + a[y1];
    //输出target,n
    cout << target << " " << n << endl;
    //输出数组
    for (auto d : a) cout << d << " ";
