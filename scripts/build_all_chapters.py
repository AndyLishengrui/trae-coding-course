...
def build_chapter(ch):
    ...
    for p in probs:
        ...
        pb.append(f'<tr><td class="spec-label"><span class="tag out">输出</span></td><td class="spec-value">{p.get("output",p.get("output_fmt","见原题"))}</td></tr>')
        pb.append(f'<tr><td class="spec-label"><span class="tag lim">范围</span></td><td class="spec-value">{p.get("range","见原题")}</td></tr>')
        pb.append('</table>')
